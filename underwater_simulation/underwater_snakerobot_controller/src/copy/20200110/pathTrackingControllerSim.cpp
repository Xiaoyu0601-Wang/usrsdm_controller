...
typedef matrix<double,3,1> column_vector;
...
private:
  ...
  double thrust_allocation_cost_fuction(const column_vector& m)
  {
    ...
    u_act_bar_tau_temp = M_inv_B*u_act_temp-bar_tau;
...
////////////////////////////////////////////////////////////////////////////////
		// dubins_init(path_start, posi_s_argmin, turning_radius, &path_v);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    // bar_tau << 1, 1, 0;//= B_inv*(targetVelB-u_c+hydro);
    ...
    // screwJointInput.data[2] = std::max(std::min(screwJointInput.data[2], max_thrustForce),
		// 							            min_thrustForce);
		/* Dynamic input */
    u_act(0) = lpf_screwVel1.update(u_act(0));
    u_act(1) = lpf_jointPos1.update(u_act(1));
    u_act(2) = lpf_screwVel2.update(u_act(2));
		...
		input_dyn(2) = std::max(std::min(-u_act(2), 3.0),-3.0);
		cout << "input_dyn: " << input_dyn << endl;
		screwJointInput.data[0] = input_dyn(0);
		screwJointInput.data[1] = input_dyn(2);
		screwJointInput.data[2] = input_dyn(1);
