...
{
public:
	Controller(const std::string& trajectory, const ros::NodeHandle& n)
    :ctrlInput_pub()
    ,hardware_pub()
		,robotState_sub()
    ,moCap_sub()
    ...
    ,lpf_screwVel1(1.0, 0.01)
    ,lpf_screwVel2(1.0, 0.01)
    ,lpf_jointPos1(1.9, 0.01)
    ...
    moCap_sub = _nh.subscribe("/RigidBody/odom", 1, &Controller::updateMoCap, this);
		...
		outFile_robotState.open("/home/michael/catkin_ws/etaDeltaTheta7.txt");//DeltaTheta

    ROS_INFO("Initialize robot");
		screwJointInput.data.resize(actuator_number);

		ROS_INFO("Initialize trajectory");
		// cout << m_trajectory << endl;
		inFile_trajectory.open("/home/michael/catkin_ws/src/underwater_simulation/underwater_snakerobot_controller/trajectory/wayPoints3.txt");
		string line;
...
private:
  ...
  double thetaErrorCal(double desired, double current)
  {
    ...
    currentWorld[2] = mod2pi(-yaw);//fmod(robotState.rotationE.z, 2 * M_PI ); //robotState.rotationE.z;
    ...
    eta = 0.5*tanh(abs(currentError(0))-2*1.0*abs(currentError(1)))+0.5;//0.5*(tanh(sqrt(pow(currentRobot(0),2)+pow(0.8*currentRobot(1),2))-1.0)+1.0);
    ...
    if ((s.CmdPosition-0.2)<0) {
      i_start = 0.0;
    }
    else
    {
      i_start = s.CmdPosition-0.2;
    }
    if ((s.CmdPosition+0.2)>s.length) {
      i_end = s.length;
    }
    else
    {
      i_end = s.CmdPosition+0.2;
    }
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    double C_c = -thetaErrorCal(path_posi_temp1[2], path_posi_temp[2])/0.02;
    if (C_c != C_c) {
      C_c = 0;
    }
    cout << "C_c: " << C_c << endl;
...
//////////////////////////////////////Print///////////////////////////////////
    ...
    cout << " C_c: " << C_c << endl;
    cout << " s.CmdVelocity: " << s.CmdVelocity << endl;
    cout << " s_dot: " << C_c << endl;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    cout << " currentRobot: " << currentRobot << endl;
  	...
  	// 	/* Velocity error in {B} frame */
		// Vector3d targetVelB;
		// targetVelB(0) = U_kinematic(0)*cos(0); //Zero represents Beta in the paper
...
int main(int argc, char **argv)
{
  ...
  nh.param<std::string>("trajectory", trajectory);
  // std::cout << trajectory << std::endl;
	/****************** Test ******************/
	std::cout << "Start: " << std::endl;
	Controller controller(trajectory, nh);
