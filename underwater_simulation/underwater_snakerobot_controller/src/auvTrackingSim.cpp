...
public:
  Controller(const std::string& trajectory, const std::string& outputDataFile, const ros::NodeHandle& n)
    ...
    ,lpf_screwVel1(9.0, 0.02)
    ,lpf_screwVel2(9.0, 0.02)
    ,lpf_jointPos1(100.0, 0.02)
    ,lpf_velu(50.0, 0.008)
    ,lpf_velv(50.0, 0.008)
    ...
    ,z(0.0, 0.0, 0.0)
    ,observer_z_dot(0.0, 0.0, 0.0)
    ,observer_z(0.0, 0.0, 0.0)
    ,observer_est_d(0.0, 0.0, 0.0)
    ,currentRobot(0.0,0.0,0.0)
    ,lastCurrentRobot(0.0,0.0,0.0)
    ,argmin_index(0.0)
    ,last_argmin_index(0.0)
    ,ReedsSheppTraj(0.5)
  {
    ros::NodeHandle _nh;
...
/////////////////////////CLean Up//////////////////////////
    ...
    while(no_waypoint == false)
    {
      for (int i = 0; i < 4; i++)
      {
        if (!(inFile_trajectory>>traj_seg_end[i]))
        {
          no_waypoint = true;
          break;
        }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    accB(0) = robotState.accelB.x;
    accB(1) = robotState.accelB.y;
    accB(2) = 0;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    **************************************/
    ...
    observer_L << 0.01, 0, 0,
                  0, 0.01, 0,
                  0, 0, 0.01;
    Vector3d observer_temp;
    observer_temp = - observer_L*observer_z;
    observer_z_dot = observer_temp + observer_L*(M_inv*hydro - bar_tau - observer_L*velB);
    if (observer_z_dot.squaredNorm() > 10)
    {
      observer_z_dot << 0, 0, 0;
    }

    observer_z(0) = integral(observer_z_dot(0), observer_z(0), ctrl_period);
    observer_z(1) = integral(observer_z_dot(0), observer_z(0), ctrl_period);
    observer_z(2) = 0;
    observer_est_d = (observer_z + observer_L*velB);
    cout << "observer_temp: " << observer_temp << endl;
    cout << "observer_z_dot: " << observer_z_dot << endl;
    cout << "observer_est_d: " << observer_est_d << endl;

    /* Feedback lineariation */
    bar_tau = alphaVelB + u_c + M_inv*hydro - observer_est_d;
...
///////////////////////////////////////////////////////////////////////////////
    ...
    // if ((-u_act(2)>0)&&(joint_angle>=max_jointAngle))
    // {
    //   u_act(2)=0;
    // }
    // else if ((-u_act(2)<0)&&(joint_angle<=-max_jointAngle))
    // {
    //   u_act(2)=0;
    // }
    // cout << "opt_input_dyn: " << result.x(0)<< ";  " << result.x(1) << endl;
////////////////////////////////////////////////////////////////////////////////
...
private:
  ...
  Matrix3d observer_L;
  Vector3d observer_z;
  Vector3d observer_z_dot;
  Vector3d observer_est_d;
