...
#        rospy.loginfo("s(): \n %s", x)
    ...
    def inverseDynamic(self) :
       ...
       #######################################################
        ...
        t[0] = self.u[0]/2 + self.u[2]/2*cos(self.joint_angle[1]) + internalForce_2to1[0]*0.3
        t[1] = self.u[2]/2*sin(self.joint_angle[1]) + internalForce_2to1[1]*0.3
        t[5] = self.link_length/2*(-self.u[2]/2*sin(self.joint_angle[1])) + internalForce_2to1[2]*0.3
                                   # +(internalForce_1to2[1]+internalForce_2to1[1])*sin(self.joint_angle[1]))
        # t[0] += innerForce[0]
        # t[1] += innerForce[1]
        # t[5] += self.mass *self.link_length*innerForce[1]/4

        t_link2[0] = self.u[2]/2 + self.u[0]/2*cos(self.joint_angle[1])+internalForce_1to2[0]*0.3
        t_link2[1] = -self.u[0]/2*sin(self.joint_angle[1])+internalForce_1to2[1]*0.3
        t_link2[5] = self.link_length/2*(-self.u[0]/2 * sin(self.joint_angle[1])) + internalForce_1to2[2]*0.3
                                         # -(internalForce_1to2[1]+internalForce_2to1[1])*sin(self.joint_angle[1]))
