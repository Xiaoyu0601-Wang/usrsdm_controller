...
class Dynamics :
	...
	self.am=rospy.get_param(self.vehicle_name + "/dynamics"+"/allocation_matrix")
        ...
        self.init_p = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_pose"))
        self.init_v = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_velocity"))
        self.v_dot_0 = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_accel"))
        self.init_joint_angle = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_joint_angle"))
        self.init_joint_angular_vel = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_joint_angular_vel"))
        self.init_joint_torque = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/initial_joint_torque"))
        ...
        # self.joint_angle = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/joint_angle"))
        # self.joint_angular_vel = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/joint_angular_vel"))
        # self.joint_angular_acc = array(rospy.get_param(self.vehicle_name + "/dynamics" + "/joint_angular_acc"))
...
#        rospy.loginfo("s(): \n %s", x)
    ...
    def addCurrentForce(self, c_v):
        ...
        Irec = matrix(rec).I
        self.current_B = dot(Irec,self.e_vc)
        self.current_B=squeeze(asarray(self.current_B))
        # print("self.current_B[0]: ",self.current_B)
        ...
        d = diag([Xu + Xuu*abs(self.current_B[0]),
                  Yv + Yvv*abs(self.current_B[1]),
                  Zw + Zww*abs(self.current_B[2])])
        self.currentForce = dot(d, self.current_B)

        c_v[0:3] = c_v[0:3] - self.current_B
        return c_v
    ...
    def inverseDynamic(self) :
        ...
        self.screwDynamics()
        ###################Robot dynamics########################
        if (self.u[0]>=0 and self.u[2]>=0) or (self.u[0]<=0 and self.u[2]<=0):
            self.c = self.coriolisMatrix(self.v)
            self.d = self.dampingMatrix(self.v)  # d[5,5] = 2*d[5,5]
            self.c_v = dot((self.c-self.d), self.v)
            self.hydro[0] = self.c_v[0]
            self.hydro[1] = self.c_v[1]
            self.hydro[5] = self.c_v[5]
            self.c_v = self.addCurrentForce(self.c_v)

            self.internal_force[5] = self.link_length/2*sin(self.joint_angle[0])*(self.damping[1]+self.quadratic_damping[1]*abs(self.v[0]))*self.v[0]/2
            self.t[0] = (self.u[0]+self.u[2])*(cos(self.joint_angle[0]))#self.u[0]+self.u[2]*cos(self.joint_angle[0])#+inner_force[0]# + internalForce_2to1[0]*0.3
            self.t[1] = (self.u[0]+self.u[2])*(sin(self.joint_angle[0]))*0.15#self.u[2]*sin(self.joint_angle[0])/8#+inner_force[1]# + internalForce_2to1[1]*0.3
            self.t[5] = self.internal_force[5]#-self.u[1]/2#-self.link_length/2*inner_force[1]
            self.v_dot_temp = dot(self.IM, (self.t-self.c_v+self.collisionForce)) #t-self.c_v-g+collisionForce   +self.collisionForce
            self.v_dot = squeeze(asarray(self.v_dot_temp)) #Transforms a matrix into an array
            if self.u[1] != 0:
                self.v_dot[5] = -self.joint_angular_acc[0]/2 + self.v_dot[5]
            # v_dot_link2[5] = self.joint_angular_acc[0] + v_dot_link2[5]
            self.c_link2 = self.coriolisMatrix(self.v_link2)
            self.d_link2 = self.dampingMatrix(self.v_link2)
            self.c_v_link2 = dot((self.c_link2-self.d_link2), self.v_link2)
            self.t_link2[0] = (self.u[0]+self.u[2])*(1+cos(self.joint_angle[0]))/2
            self.t_link2[1] = -(self.u[0]+self.u[2])*(1+sin(self.joint_angle[0]))/2# + sin(self.joint_angle[0])*inner_force[0] - cos(self.joint_angle[0])*inner_force[1]# + internalForce_1to2[1]*0.3
            self.t_link2[5] = self.u[1]+self.internal_force[5]
            self.v_dot_link2_temp = dot(self.IM, (self.t_link2-self.c_v_link2)) #+self.collisionForce
            self.v_dot_link2 = squeeze(asarray(self.v_dot_link2_temp)) #Transforms a matrix into an array
            # if self.u[1] == 0:
            #     v_dot[5] = (v_dot[5] + v_dot_link2[5])/2
            # v_dot_link2 = v_dot

        elif (self.u[0]>0 and self.u[2]<0) or (self.u[0]<0 and self.u[2]>0):
            self.c = self.coriolisMatrix(self.v)
            self.d = self.dampingMatrix(self.v)
            self.c_v = dot((self.c-self.d), self.v)
            self.hydro[0] = self.c_v[0]
            self.hydro[1] = self.c_v[1]
            self.hydro[5] = self.c_v[5]
            ...
            self.internal_force[0] = (self.u[0]-self.u[2])*sin(self.joint_angle[0]/2)*sin(self.joint_angle[0]/2)#(self.u[0]+self.u[2])*cos(self.joint_angle[0]/2)*cos(self.joint_angle[0]/2)
            self.internal_force[1] = -(self.u[0]-self.u[2])*sin(self.joint_angle[0]/2)*cos(self.joint_angle[0]/2)+(self.u[0]+self.u[2])*cos(self.joint_angle[0]/2)*cos(self.joint_angle[0]/2)
            self.internal_force[5] = -self.link_length/2*(self.u[0]+self.u[2])*cos(self.joint_angle[0]/2)*sin(self.joint_angle[0]/2)*0.25
            self.t[0] = self.internal_force[0]#+self.u[2]*cos(self.joint_angle[0])#+inner_force[0]# + internalForce_2to1[0]*0.3
            self.t[1] = self.internal_force[1]#self.u[2]*sin(self.joint_angle[0])/8#+inner_force[1]# + internalForce_2to1[1]*0.3
            self.t[5] = self.internal_force[5]#-self.link_length/2*inner_force[1]
            self.v_dot_temp = dot(self.IM, (self.t-self.c_v+self.collisionForce)) #t-self.c_v-g+collisionForce   +self.collisionForce
            self.v_dot = squeeze(asarray(self.v_dot_temp)) #Transforms a matrix into an array
            # temp = (self.u[0]-self.u[2])*sin(self.joint_angle[0]/2)
            # v_dot[0] = temp/self.mass*sin(self.joint_angle[0]/2) + v_dot[0]
            # v_dot[1] = -temp/self.mass*cos(self.joint_angle[0]/2) + v_dot[1]
            if self.u[1] != 0:
                self.v_dot[5] = -self.joint_angular_acc[0]/2 + self.v_dot[5]
            ...
            self.v_dot_link2_temp = dot(self.IM, (self.t_link2-self.c_v_link2)) #+self.collisionForce
            self.v_dot_link2 = squeeze(asarray(self.v_dot_link2_temp)) #Transforms a matrix into an array
       #  internalForce_1to2 = dot(self.transMatrixI2B(self.joint_angle[0]), self.c_v)
       #  internalForce_2to1 = dot(self.transMatrixB2I(self.joint_angle[0]), self.c_v_link2)
       #  inner_force = array(zeros(6))
       #  inner_force[0] = self.mass/2*(self.v_dot[0]-self.v[1]*self.v[5])-self.u[0]+d[0,0]
       #  inner_force[1] = self.mass/2*(self.v_dot[1]+self.v[0]*self.v[5])+d[1,1]
       # #######################################################
       #  t = array(zeros(6))
       ...
       #  t[0] = self.u[0]+self.u[2]*cos(self.joint_angle[0])+inner_force[0]# + internalForce_2to1[0]*0.3
       #  t[1] = self.u[2]*sin(self.joint_angle[0])+inner_force[1]# + internalForce_2to1[1]*0.3
       #  t[5] = -self.u[1]-self.link_length/2*inner_force[1]          #self.link_length*(-self.u[2]*sin(self.joint_angle[0])) + internalForce_2to1[2]*0.3
       #                             # +(internalForce_1to2[1]+internalForce_2to1[1])*sin(self.joint_angle[0]))
       #
       #  t_link2[0] = self.u[2] + self.u[0]*cos(self.joint_angle[0]) - cos(self.joint_angle[0])*inner_force[0] - sin(self.joint_angle[0])*inner_force[1]# + internalForce_1to2[0]*0.3
       #  t_link2[1] = - self.u[0]*sin(self.joint_angle[0]) + sin(self.joint_angle[0])*inner_force[0] - cos(self.joint_angle[0])*inner_force[1]# + internalForce_1to2[1]*0.3
       #  t_link2[5] = self.link_length/2*inner_force[0]*sin(self.joint_angle[0])- self.link_length/2*inner_force[1]*cos(self.joint_angle[0])           #self.link_length/2*(-self.u[0]/2 * sin(self.joint_angle[0])) + internalForce_1to2[2]*0.3
                                         # -(internalForce_1to2[1]+internalForce_2to1[1])*sin(self.joint_angle[0]))
        ...
        # self.innerForce[3] = self.joint_angular_acc[0]*4.5
        #########################
        # v_dot_link2 = v_dot
        # v_dot_link2 = dot(self.IM, (t_link2-self.c_v_link2)) #+self.collisionForce
        # v_dot_link2 = squeeze(asarray(v_dot_link2)) #Transforms a matrix into an array

        # v_dot[5] = -self.joint_angular_acc[0] + v_dot[5]
        # v_dot_link2[5] = self.joint_angular_acc[0] + v_dot_link2[5]#temp

        time_flag += 1
        if time_flag == 100 :
            print("internal_force[5]: ",self.internal_force[5])
            print("self.joint_angular_acc[0]: ",self.joint_angular_acc[0])
            print("self.joint_angular_vel[0]: ",self.joint_angular_vel[0])
            print("self.joint_angle[0]: ",self.joint_angle[0])
            ...
            # print("self.c_v_link2: ",self.c_v_link2)
        ...
        accel = [self.v_dot, self.v_dot_link2]
        # print(accel[0])
    ...
    def kinematics(self) :
        ...
        # Irec = matrix(rec).I
        # current_B = dot(Irec,self.e_vc)
        # print("current_B: ",current_B)
        # self.v[0:3] = self.v[0:3] + current_B
        # print("self.v: ",self.v)
    ...
    def jointDynamics(self):
        #calculate joint angular acceleration
        self.joint_angular_acc[0] = (self.u[1])/self.I_joint
                                    # (self.u[1]+self.Ma[5,5]*self.joint_angular_acc[0]
                                    # +self.quadratic_damping[5]*abs(self.joint_angular_vel[0])*self.joint_angular_vel[0]
                                    # +self.damping[5]*self.joint_angular_vel[0])/self.I_joint
        # self.joint_angular_acc[0] = self.u[1]/self.I_joint
        if self.joint_angle[0] >= self.joint_angle_limit[0] and self.u[1]>0:
            self.u[1] = 0
            self.joint_angular_acc[0] = 0
            self.joint_angular_vel_drift[0] = self.joint_angular_vel[0]
            self.joint_angular_vel[0] = 0
            self.joint_angle[0] = self.joint_angle_limit[0]
        elif self.joint_angle[0] <= -self.joint_angle_limit[0] and self.u[1]<0:
            self.u[1] = 0
            self.joint_angular_acc[0] = 0
            self.joint_angular_vel_drift[0] = self.joint_angular_vel[0]
            self.joint_angular_vel[0] = 0
            self.joint_angle[0] = -self.joint_angle_limit[0]
        # calculate joint angular velocity and angle
        self.joint_angular_vel[0] = self.integral(self.joint_angular_acc[0],
                                                  self.joint_angular_vel[0],
                                                  self.period)
        self.joint_angle[0] = self.integral(self.joint_angular_vel[0],
                                            self.joint_angle[0],
                                            self.period)
        if self.joint_angle[0] >= self.joint_angle_limit[0] and self.u[1]>0:
            self.joint_angle[0] = self.joint_angle_limit[0]
    ...
    def screwDynamics(self):
        if self.u[0] > 0:
            self.screw_angular_vel[0] = radians(sqrt(abs(self.u[0])/self.screw_gain)*6)
        else:
            self.screw_angular_vel[0] = -radians(sqrt(abs(self.u[0])/self.screw_gain)*6)
        if self.u[2] > 0:
            self.screw_angular_vel[1] = radians(sqrt(abs(self.u[2])/self.screw_gain)*6)
        else:
            self.screw_angular_vel[1] = -radians(sqrt(abs(self.u[2])/self.screw_gain)*6)
        for i in range (2):
    ...
    def pubPose(self, event):
        ...
        pose.angular_vel.x = 0
        pose.angular_vel.y = 0
        pose.angular_vel.z = self.v[5]

        pose.joint_angle = self.joint_angle[0]
    ...
    def pubScrew(self, event):
        ...
        # screw.velocity = []
        # screw.velocity = [self.screw_angular_vel[0],self.screw_angular_vel[0],
        #                   self.joint_angular_vel[0],
        #                   self.screw_angular_vel[1],self.screw_angular_vel[1]]
        screw.effort = []
    ...
    #   Init joint state
        self.joint_angle = self.init_joint_angle
        self.joint_angular_vel = self.init_joint_angular_vel
        self.joint_angular_vel_drift = array(zeros(2))
        self.joint_angular_acc = dot(self.I_joint,self.init_joint_torque)
    #   Init link pose and velocity
        self.t = array(zeros(6))
        self.t_link2 = array(zeros(6))
        self.internal_force = array(zeros(6))

        self.c_v = array(zeros(6))
        self.v_dot = self.init_v   #accleration in {B}
        self.v = self.init_v   #velocity in {B}
        self.p = self.init_p   #position in {I}
        self.v_dot_link2 = array(zeros(6))
        self.v_link2 = self.init_v
        self.p_link2 = [self.p[0]-self.link_length/2*(cos(self.p[5])+cos(self.p[5]+self.joint_angle[0])),
                        self.p[1]-self.link_length/2*(sin(self.p[5])+sin(self.p[5]+self.joint_angle[0])),
                        0, 0, 0, self.p[5]+self.joint_angle[0]]
        ...
        self.Ma = rospy.get_param(self.vehicle_name + "/dynamics" + "/Ma")
        self.Ma = array(self.Ma).reshape(6, 6)

        self.M = Mrb - self.Ma    # mass matrix: Mrb + Ma
...
#        rospy.loginfo("Inverse Mass Matrix: \n%s", str(self.IM))
    ...
    def iterate(self):
        ...
        if self.joint_angular_vel_drift[0] != 0:
            self.v[5] = self.v[5] + self.joint_angular_vel_drift[0]*0.25#*0.315
            self.joint_angular_vel_drift[0] = 0
        # self.v[5] = -self.joint_angular_vel[0] + self.v[5]
        self.p_dot = self.kinematics()
        self.p = self.integral(self.p_dot, self.p, self.period)

        self.v_link2[0] = self.link_length/2*sin(self.joint_angle[0])*self.joint_angular_vel[0]+self.v[0]*cos(self.joint_angle[0])+self.v[1]*sin(self.joint_angle[0])
        self.v_link2[1] = -self.link_length/2*cos(self.joint_angle[0])*self.joint_angular_vel[0]-self.v[0]*sin(self.joint_angle[0])+self.v[1]*cos(self.joint_angle[0])
        self.v_link2[5] = self.v[5] + self.joint_angular_vel[0]
        ...
        self.p_link2[5] = self.p[5]+self.joint_angle[0]
        ...
        # self.v_link2 = dot(self.transMatrixI2B(self.joint_angle[0]),self.v)
        # self.v_dot_link21 = dot(self.transMatrixB2I(self.joint_angle[0]),self.v_dot)
